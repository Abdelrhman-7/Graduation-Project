...
class LocalBookingStore {
  ...
  Future<List<BookingModel>> getBookingsForPatient({
    ...
    return all
        .where((b) => _matchesPatient(b, patientName, patientEmail))
        .toList()
      ..sort((a, b) => (b.createdAt ?? '').compareTo(a.createdAt ?? ''));
  }
  ...
  Future<List<BookingModel>> getUnreadNotificationsForPatient({
    ...
    final bookings = await getBookingsForPatient(
      patientName: patientName,
      patientEmail: patientEmail,
    );

    return bookings.where((b) => b.notificationUnread && !b.isPending).toList()
      ..sort((a, b) => (b.createdAt ?? '').compareTo(a.createdAt ?? ''));
  }
  ...
  Future<bool> updateStatus(int bookingId, String status) async {
    ...
    final notifyPatient =
        wasPending && (status == 'Approved' || status == 'Rejected');
